...
(defn voronoi [{:keys [config-opts] :as Φ} {:keys [width height]}]
  (r/create-class
   {:component-did-mount #(side-effect! Φ :home-page/voronoi-did-mount {:node (r/dom-node %) :width width :height height})
    ;; TODO re-drawing with newly selected points
    :component-did-update (fn [_ _] (side-effect! Φ :home-page/voronoi-did-update))
    :reagent-render (fn []
                      (let [viewport (track Φ l/view-single
                                            (in [:viewport]))]
...
(defmethod process-side-effect :home-page/voronoi-did-mount [Φ id {:keys [node width height]}]
  (let [data (clj->js (mapv (fn [] {:x (rand-int width) :y (rand-int height)}) (range 100)))
